...
namespace Lsr.Bio.HtsAnalysis.Simulations {
    public class DataSimulationLoader {
        ...
        public ArrayedScreenData LoadFromReader<T>(TextReader inReader, string inputSource, int numPlates, int numRows, int numCols) {
            ...
            PlatesetInfo<Reagent> reagents = new PlatesetInfo<Reagent>(result.ReagentPlatesetName, provenance,
                numPlates, numRows, numCols);

            this._ClearState();
            while ((line = inReader.ReadLine()) != null) {
                fields = line.Split(SimulationUtilities.DELIMITER.ToCharArray());
        ...
        private void _SetStateFromHeaderLine(string[] fields, int index) {
            ...
            this._ValueTypeIndex = fields.ToList().IndexOf(SimulationUtilities.VALUE_TYPE_HEADER);
